...
def main():
    ...
    num_processes = 150
    num_files_per_segment = 300  # number of files to process in each batch
    
    # Split file list into segments
    for segment_index, start in enumerate(range(0, len(nc_file_list), num_files_per_segment)):
        end = start + num_files_per_segment
        current_file_list = nc_file_list[start:end]
        chunk_size = len(current_file_list) // num_processes + (len(current_file_list) % num_processes > 0)

        print(f'Calculating.... Segment {segment_index + 1}')
        pool = Pool(processes=num_processes)
        results = pool.starmap(process_files, [(current_file_list[i:i + chunk_size], ref_points, data_shape) for i in range(0, len(current_file_list), chunk_size)])
        pool.close()
        pool.join()
        ...
        for r in range(final_timestamp_median.shape[0]):
            for c in range(final_timestamp_median.shape[1]):
                if len(final_timestamp_median[r, c]) > 1:
                    final_timestamp_median[r, c].sort()
                    differences = np.diff(final_timestamp_median[r, c])
                    median_diff = np.median(differences)
                    median_time_differences[r, c] = median_diff
                else:
                    median_time_differences[r, c] = None

        # Save to CSV with segment identifier
        np.savetxt(f"/data/CYGNSS/data_counts_csv/CYGNSS_angle_sum_{resol}_seg_{segment_index+1}.csv", final_angle_sum, delimiter=',')
        np.savetxt(f"/data/CYGNSS/data_counts_csv/CYGNSS_angle_sum_sq_{resol}_seg_{segment_index+1}.csv", final_angle_sum_sq, delimiter=',')
        np.savetxt(f"/data/CYGNSS/data_counts_csv/CYGNSS_data_count_{resol}_seg_{segment_index+1}.csv", final_data_count, delimiter=',')    
        np.savetxt(f"/data/CYGNSS/data_counts_csv/CYGNSS_median_time_diff_{resol}_seg_{segment_index+1}.csv", median_time_differences, delimiter=',')

        print(f'Finished processing segment {segment_index + 1}. Data count shape:', final_data_count.shape)
