...
class BandReader:
    ...
    def get_band_with_transform(self, band):
        ...
        with rasterio.open(path) as src:
            ...
            mask_list = ['cirrus', 'coastal', 'qa']

            if self.product == 'L30':
                mask_list.append('tir1')
                mask_list.append('tir2')

                if band not in mask_list:
                    ...
                    data = np.where(data == self.fill_value, np.nan, data * self.scale_factor_tir)

            if self.product == 'S30':
                mask_list.append('water_vapor')
    ...
    def _read_band(self, band_file):
        ...
        if self.product == 'L30':
            ...
            with rasterio.open(path) as src:
                ...
                elif band_file == self.band_files['tir1'] or band_file == self.band_files['tir2']:
